...
export default function ScoutingMap({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4">
      <div className="w-full max-w-5xl bg-zinc-900 border-2 border-sky-500/30 rounded-[3rem] p-8 relative overflow-hidden">
        
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-3xl font-black italic text-sky-400 uppercase tracking-tighter italic">Global Talent Tracker</h2>
          <button onClick={onClose} className="bg-white/10 hover:bg-red-600 px-4 py-2 rounded-full text-[10px] font-bold uppercase transition-all">Close Map [X]</button>
        </div>

        {/* WORLD MAP CONTAINER */}
        <div className="relative aspect-video bg-[url(\"https://www.transparenttextures.com/patterns/stardust.png\")] bg-zinc-950 rounded-2xl border border-white/5 overflow-hidden">
          {/* SIMULATED MAP GRID */}
          ...
          {pins.map((pin) => (
            <div key={pin.city} className="absolute group" style={{ top: pin.top, left: pin.left }}>
              ...
              {/* TOOLTIP */}
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black border border-sky-500 p-2 rounded text-[8px] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all">
        ...
        <div className="mt-8 flex justify-around text-[10px] font-mono text-gray-500 uppercase tracking-widest">
          ...
          <p className="text-sky-500 animate-pulse">Scanning Global Markets...</p>
