    ...
    <div className="min-h-screen bg-black text-white p-6 font-mono">
      ...
      <div className="flex justify-between items-start border-b border-[#d4af37]/20 pb-6 mb-10">
        ...
        <div className="bg-[#d4af37]/10 border border-[#d4af37]/30 px-4 py-2 rounded-lg text-right">
           <p className="text-[8px] text-[#d4af37] uppercase font-bold tracking-widest">Security Status</p>
           <p className="text-xs font-black text-white uppercase italic">Elite Verified</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {["Technical", "Medical", "Scouting", "Intelligence"].map((dept) => (
          <div key={dept} className="bg-zinc-900/40 border border-white/5 p-8 rounded-[2.5rem] hover:shadow-[0_0_30px_rgba(212,175,55,0.1)] transition-all">
            <h2 className="text-2xl font-black uppercase mb-6 text-gray-400 italic">{dept}</h2>
            <div className="grid grid-cols-2 gap-3">
              <Link href="/admin/manage" className="bg-zinc-800 hover:bg-emerald-600 p-4 rounded-2xl text-[10px] font-black uppercase text-center transition-all">Credentials</Link>
              
              {/* TRIGGER SELF DESTRUCT */}
              <button 
                onClick={() => setIsPurging(true)}
                className="bg-zinc-800 hover:bg-red-700 p-4 rounded-2xl text-[10px] font-black uppercase text-center transition-all"
              >
                Delete Data
              </button>
